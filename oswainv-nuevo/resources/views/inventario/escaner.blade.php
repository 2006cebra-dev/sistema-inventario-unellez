...
<head>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        .oswa-toast i { font-size: 1.4rem; color: #E50914; flex-shrink: 0; filter: drop-shadow(0 0 6px rgba(229,9,20,0.4)); }
        .oswa-toast .toast-progress { position: absolute; bottom: 0; left: 0; height: 3px; background: linear-gradient(90deg, #E50914, #ff6b6b); animation: oswaToastProgress 3s linear forwards; border-radius: 0 2px 0 0; }
        .oswa-toast.removing { animation: oswaToastOut 0.4s cubic-bezier(0.55, 0, 0.55, 0.4) forwards; }
        .oswa-toast.removing .toast-progress { animation-play-state: paused; }
        @keyframes oswaToastIn { from { transform: translateY(40px) scale(0.95); opacity: 0; } to { transform: translateY(0) scale(1); opacity: 1; } }
        @keyframes oswaToastOut { from { transform: translateY(0) scale(1); opacity: 1; } to { transform: translateY(20px) scale(0.95); opacity: 0; } }
